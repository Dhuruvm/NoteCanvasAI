...
/**
 * Enhanced summarization using multiple AI models
 */
export async function summarizeWithMultipleModels(
  content: string,
  settings: AISettings,
  pdfBuffer?: Buffer
): Promise<ProcessedNote> {
  console.log('Starting enhanced multi-model processing...');
  
  try {
    // First, get the basic Gemini processing
    const basicProcessing = await processBasicGemini(content, settings, pdfBuffer);
    
    // Then enhance with Hugging Face models
    const designStyle = settings.designStyle || 'modern';
    const multiModelResult = await processWithMultipleModels(content, designStyle);
    
    // Combine results
    const enhancedNote: ProcessedNote = {
      ...basicProcessing,
      enhancedContent: multiModelResult.enhancedContent,
      designLayout: multiModelResult.designLayout,
      tableStructure: multiModelResult.tableStructure,
      metadata: {
        ...basicProcessing.metadata,
        aiModelsUsed: [
          'gemini-2.5-flash',
    ...
    console.error('Multi-model processing failed, falling back to basic Gemini:', error);
    // Fallback to basic Gemini processing
    return await processBasicGemini(content, settings, pdfBuffer);
  }
}

/**
 * Basic Gemini processing (original implementation)
 */
async function processBasicGemini(
  content: string,
  settings: AISettings,
  pdfBuffer?: Buffer
): Promise<ProcessedNote> {
  // This is the original Gemini processing logic moved from the main function
  try {
    const systemPrompt = `You are an expert educational content analyzer and note generator. 
Your task is to transform the provided content into well-structured study notes.
